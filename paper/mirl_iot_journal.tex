...
Fig.~\ref{fig:reward} presents the episode reward progression throughout the training process. The reward trajectory exhibits steady improvement during early episodes followed by asymptotic stabilization, consistent with successful policy gradient optimization. Application of moving average smoothing (window size 30) isolates the underlying learning trend from inherent episode-to-episode stochasticity.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig3_reward_curve.pdf}
\caption{Episode reward during SGAC training. The smoothed trajectory (solid) exhibits monotonic improvement followed by convergence, with shaded region indicating variance bounds.}
\label{fig:reward}
\end{figure}

\subsection{Performance Floor Validation}

Fig.~\ref{fig:floor} tracks floor mechanism activation frequency during training. The activation rate exhibits monotonic decrease from elevated values in early episodes---when the undertrained policy proposes corrections that degrade performance---to near-zero in later episodes as the policy learns to produce beneficial corrections. Evaluation on held-out test scenarios confirms:
\begin{itemize}
...
\caption{Floor mechanism activations per episode during training. Monotonically decreasing activation frequency indicates the policy learns corrections that consistently improve upon the SCA baseline.}
\label{fig:floor}
\end{figure}
