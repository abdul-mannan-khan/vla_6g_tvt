...
\title{Distilling Evolutionary Optimization into Compact Language Models for Real-Time UAV Relay Positioning in Vehicular THz Networks}

\author{Abdul~Manan~KHAN,~\IEEEmembership{Senior Member,~IEEE}%
\thanks{Abdul~Manan KHAN is with the Dyson School of Design Engineering, Imperial College London, London, the United Kingdom and with the School of Computing Engineering, University of West London, London, the United Kingdom (e-mail: khanabd@uwl.ac.uk).}%
\thanks{Manuscript received Month DD, 2026; revised Month DD, 2026.}}

\markboth{IEEE Transactions on Vehicular Technology}%
{KHAN: Distilling Evolutionary Optimization into Compact Language Models for Real-Time UAV Relay Positioning}

\maketitle

% ============================================================
\begin{abstract}
Positioning an unmanned aerial vehicle (UAV) relay for vehicular terahertz (THz) backhaul requires both solution quality and adaptability to evolving mission contexts---obstacle maps, weather conditions, multi-site topologies---that rigid feature-engineered models cannot absorb without redesign. This paper introduces \emph{optimizer distillation}: a differential-evolution solver generates 10{,}000 relay-placement solutions offline, and a TinyLlama-1.1B language model is trained to reproduce them via quantized low-rank adaptation (QLoRA). The resulting Verbal Language Agent (VLA) reads textual scenario descriptions and writes relay coordinates, giving it a modality-agnostic input interface. A multilayer perceptron (MLP) baseline achieves higher raw throughput (170.2 vs.\ 134.0\,Mbps) at sub-millisecond latency; however, when building obstructions are introduced, fine-tuning the VLA on just 500 obstacle-aware labels lifts it to 142.6\,Mbps---surpassing the blind MLP (137.8\,Mbps)---with no architectural change, whereas the MLP would require feature-vector redesign and full retraining. This \emph{input extensibility} is the core advantage. Across 100 evaluation scenarios (3--7 vehicular terminals), the VLA exceeds the centroid heuristic by 13.2\% ($p{=}0.0073$) with a Jain fairness of 0.963. Merging low-rank adaptation (LoRA) adapters into 16-bit floating-point (FP16) weights reduces inference from 10.4\,s to 822\,ms on a laptop graphics processing unit (GPU); a preliminary TensorRT conversion further reduces this to an estimated 150--200\,ms. A Hybrid controller combining centroid tracking (100\,ms) with periodic VLA overrides achieves 145.5\,Mbps at 0\,km/h and comes within 3\% of the full optimizer at 120\,km/h.
\end{abstract}

\begin{IEEEkeywords}
Differential evolution, energy efficiency, language model, optimizer distillation, QLoRA, terahertz communications, UAV relay, vehicular networks.
\end{IEEEkeywords}
...
\IEEEPARstart{S}{ixth-generation} (6G) vehicular standards call for peak data rates above 1\,Tbps and air-interface latencies under 100\,$\mu$s \cite{rappaport2019}. Reaching those numbers pushes vehicular-to-everything (V2X) backhaul from sub-6\,GHz sidelinks toward millimeter-wave and terahertz (THz) bands, where contiguous bandwidths of 10\,GHz or more can absorb the aggregate traffic of dense platoons, autonomous-driving sensor feeds, and in-vehicle extended-reality streams \cite{akyildiz2022, chaccour2022}. The difficulty at 300\,GHz is that free-space path loss plus molecular absorption limits reliable links to a few hundred meters, leaving coverage holes wherever roadside unit (RSU) density is too low \cite{sarieddeen2021}.

Unmanned aerial vehicles (UAVs) can fill these holes: a single relay drone exploits the favorable air-to-ground channel to set up near-line-of-sight paths between an RSU and a moving cluster \cite{zeng2016wireless}. The central problem is \emph{where to position it}. The relay position must jointly maximize aggregate throughput, per-user fairness, and quality-of-service (QoS) coverage, and it must be recomputed as the cluster moves. At one extreme, centroid-based heuristics \cite{zeng2016throughput} run in microseconds but bake in geometric assumptions that break when user positions are unevenly distributed. At the other extreme, evolutionary solvers such as differential evolution \cite{storn1997} get close to the Pareto front but need hundreds of milliseconds per query, which can exceed the coherence time of a highway platoon.
...
A fixed-architecture regressor (e.g., a multilayer perceptron) can achieve higher raw throughput at the same data budget, but its rigid feature vector cannot accommodate new scenario modalities---obstacle maps, weather conditions, multi-RSU contexts---without architectural redesign and full retraining. The language model's text-in/text-out interface absorbs such changes as additional prompt sentences, requiring only lightweight fine-tuning. This \emph{input extensibility} is the primary motivation for choosing a language model over conventional regressors. Experiments confirm the trade-off: the VLA statistically beats centroid heuristics, and under building obstructions it surpasses the blind MLP after fine-tuning on just 500 labels.

Learning from optimization traces (\emph{learning to optimize}~\cite{bengio2021}) is established in operations research but has not been applied to continuous spatial positioning in wireless networks. Recent LLM-based drone navigation~\cite{khan2025llmnav} and explainable RL for 6G~\cite{aranacatania2025xdrl} show growing interest in language-model interfaces for aerial optimization. To the best of the author's knowledge, this is the first study that distills an evolutionary relay-positioning solver into a language model for vehicular THz networks, with measured sub-second inference after LoRA merging.

Six contributions are made.
\begin{enumerate}
    \item \textbf{Input extensibility} is demonstrated as the distilled model's defining advantage over fixed-feature regressors. Adding building obstructions requires only appending text to the prompt and fine-tuning on 500 labels (${\sim}$16\,min); the result (142.6\,Mbps) surpasses the blind MLP (137.8\,Mbps). An MLP would need feature-vector redesign and full retraining to absorb the same modality.

    \item \textbf{Optimizer distillation} is formalized for relay positioning: differential-evolution outputs are cast as supervised sequence-prediction targets for a compact language model. The scheme generalizes to any network optimization where offline solution quality outstrips real-time budgets.

    \item A \textbf{100-scenario evaluation} with nine baselines---MLP, two DeepSets variants, three DRL agents (PPO, SAC, TD3), plus Random, Static and Analytical heuristics---provides Bonferroni-corrected paired $t$-tests, bootstrap confidence intervals and Cohen's $d$ effect sizes across all comparisons. The MLP achieves higher raw throughput (170.2\,Mbps vs.\ 134.0\,Mbps), confirming an architectural gap that reverses under input-format changes.

    \item \textbf{Inference latency} is measured across four configurations down to 822\,ms (merged FP16 greedy decoding), with a preliminary TensorRT estimate of 150--200\,ms for vehicular edge deployment.

    \item A \textbf{Hybrid mobility controller} combining centroid tracking (100\,ms) with periodic VLA overrides at 0--120\,km/h achieves 145.5\,Mbps at 0\,km/h and comes within 3\% of the full optimizer at highway speed.

    \item \textbf{Ablation and energy analyses} confirm log-linear data scaling, stable performance across user counts (3--7), and a 13.2\% throughput-per-Joule gain over the centroid heuristic.
\end{enumerate}

Section~\ref{sec:related} reviews related work on vehicular UAV relaying, THz propagation and learning-based optimization. Section~\ref{sec:system} defines the system model. The three-stage distillation pipeline is detailed in Section~\ref{sec:method}. Section~\ref{sec:results} presents the full experimental campaign: latency benchmarks, MLP/DRL/DeepSets baselines (including attention-pooling and SAC), input extensibility with obstacle-aware fine-tuning, weight sensitivity, ablation, hybrid vehicular mobility, Pareto analysis and energy efficiency. Section~\ref{sec:conclusion} concludes.
...
Kong \textit{et al.}~\cite{kong2023multi} train multi-agent RL for UAV obstacle avoidance; Wu \textit{et al.}~\cite{wu2024netllm} adapt LLMs to network tasks; Khan \textit{et al.}~\cite{khan2025llmnav} feed obstacle context directly to an LLM for drone trajectory planning. DeepSets~\cite{zaheer2017deepsets} provides permutation-invariant set-function encoding for variable-size inputs. The \emph{learning to optimize} (L2O) paradigm~\cite{bengio2021} trains fast surrogates on slow solvers but has not been applied to continuous spatial positioning in wireless networks. The present work extends L2O by exploiting a language model's text interface for input extensibility.
...
\begin{itemize}
    ...
    \item \textbf{MLP}: a three-hidden-layer perceptron ($37 \to 256 \to 128 \to 64 \to 3$) with batch normalization, trained via MSE regression on 8{,}000 optimizer solutions (Section~\ref{sec:mlp_drl}).
    ...
    \item \textbf{TD3}: a Twin Delayed Deep Deterministic Policy Gradient (DDPG) agent with clipped double-Q learning and target policy smoothing, same architecture and budget (Section~\ref{sec:mlp_drl}).
...
\begin{table*}[!t]
    ...
    \begin{tabular}{@{}l c c c c c c c@{}}
        ...
        \bottomrule
        \multicolumn{8}{@{}l}{\scriptsize $^\dagger$Merged FP16 greedy decoding (Sec.~\ref{sec:latency}). Within 1\% of 4-bit baseline throughput. Baseline latency: 10.4\,s.} \\
        \multicolumn{8}{@{}l}{\scriptsize $^\ddagger$Inference-only baselines; no UAV flight simulation.} \\
    \end{tabular}
\end{table*}

The VLA averages 134.0\,Mbps (bootstrap 95\% CI: [125.2, 143.3]) versus 118.4\,Mbps ([110.1, 126.8]) for the centroid heuristic, a 13.2\% gap confirmed by paired $t$-test ($t{=}2.74$, $p{=}0.0073$; Bonferroni-corrected $p{=}0.044$; Cohen's $d{=}0.27$). It recovers 70.8\% of the evolutionary optimum (189.3\,Mbps) from a single forward pass and surpasses the best DRL baseline (SAC, 114.5\,Mbps) by 17\%.
...
\emph{Training set size.} Throughput scales log-linearly: $T \approx 10.7 \ln K + 52.8$ ($R^2{=}0.998$). Extrapolation suggests 10{,}000 samples would yield ${\sim}$153\,Mbps (80.8\% of the optimum), indicating a data-limited rather than capacity-limited gap.
...
\begin{equation}
    \rho = \frac{T_\text{VLA} - T_\text{Analytical}}{T_\text{Optimized} - T_\text{Analytical}} = \frac{134.0 - 118.4}{189.3 - 118.4} = 0.220
...
Using Eq.~\eqref{eq:energy_total} with $\Delta t = 5$\,min, hovering energy (${\sim}$50.5\,kJ) dominates transit energy (${\sim}$1.0--1.3\,kJ) by ${\sim}40\times$, so the efficiency ranking collapses to a throughput ranking. The VLA delivers 2.651\,Mbps/kJ---13.2\% more throughput per Joule than Analytical (2.342\,Mbps/kJ). For shorter missions or longer transit distances the two metrics would diverge.


\subsection{Simulation Deployment Verification}
\label{sec:simulation}

The full system is deployed in two simulation environments on Ubuntu~24.04 / ROS\,2~Jazzy.

An RViz node publishes MarkerArray messages at 15\,Hz, rendering the THz RSU, vehicular terminals with throughput labels, and the UAV relay with color-coded link beams (Fig.~\ref{fig:rviz_demo}). Separately, a Gazebo Sim instance runs an x500 quadrotor commanded via \texttt{TrajectorySetpoint} messages at 10\,Hz in a simulated urban environment with buildings, a 30\,m RSU tower, and road infrastructure (Fig.~\ref{fig:gazebo_demo}). Together, the two environments confirm that the distilled model's positions are reachable by a quadrotor (convergence within 3--4\,s per setpoint) and that the ROS\,2 pipeline works end-to-end.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{figures/rviz_simulation_tvt.png}
    \caption{RViz 3D visualization. THz links are color-coded by achievable rate; coverage domes show per-terminal QoS satisfaction.}
    \label{fig:rviz_demo}
\end{figure}
...
\begin{figure}[!t]
    ...
    \caption{Gazebo physics simulation with an x500 quadrotor validating relay-position feasibility under realistic flight dynamics.}
...
\textbf{DeepSets.} A permutation-invariant set-function architecture~\cite{zaheer2017deepsets}: per-user encoder $\phi: \mathbb{R}^5 \to \mathbb{R}^{128}$, sum-pooling, decoder $\rho: \mathbb{R}^{137} \to \mathbb{R}^3$. A \textbf{DeepSets-Attn} variant replaces sum-pooling with 4-head attention pooling.
...
To test extensibility, 1--3 random building obstructions (20\,dB NLoS penalty) are added per scenario. Table~\ref{tab:extensibility} compares six methods on 50 obstacle scenarios.

\begin{table}[!t]
    \centering
    \caption{Performance Under Building Obstructions (50 Scenarios)}
    \label{tab:extensibility}
    \footnotesize
    \begin{tabular}{@{}lccc@{}}
        \toprule
        \textbf{Method} & \textbf{Thpt.\ (Mbps)} & \textbf{Fairness} & \textbf{Cov.\ (\%)} \\
        \midrule
        Analytical (blind) & 101.3 & 0.855 & 29.6 \\
        VLA-zeroshot$^\dagger$ & 129.3 & 0.959 & 44.9 \\
        VLA (blind) & 135.1 & 0.949 & 50.1 \\
        MLP-2K (blind) & 137.8 & 0.958 & 49.6 \\
        \textbf{VLA-Obs} & \textbf{142.6} & 0.941 & \textbf{52.7} \\
        Oracle (obs-aware DE) & 173.0 & 0.950 & 75.6 \\
        \bottomrule
        \multicolumn{4}{@{}l}{\scriptsize $^\dagger$Original VLA with obstacle text in prompt, no fine-tuning.} \\
    \end{tabular}
\end{table}

Blind MLP and VLA perform similarly (${\sim}$136\,Mbps). Zero-shot obstacle prompting \emph{degrades} VLA throughput by 4.3\%, but fine-tuning on 500 obstacle-aware labels (2 epochs, ${\sim}$16\,min) lifts VLA-Obs to 142.6\,Mbps---surpassing the blind MLP by 3.5\% and closing 20\% of the gap to the oracle. No architectural change is needed; the MLP would require feature-vector redesign, retraining, and re-validation.
...
% ============================================================
% DISCUSSION AND LIMITATIONS — commented out to reduce page count
% ============================================================
%\section{Discussion}
%\label{sec:discussion}
%
%\subsection{Vehicular Deployment Considerations}
%
%Vehicular deployment adds constraints that static network optimization can ignore. Cluster coherence times run 1--10\,s in urban traffic (30--60\,km/h) and drop to 0.1--1\,s on highways (100--130\,km/h). The merged FP16 latency of 822\,ms fits the urban regime comfortably. The mobility data in Section~\ref{sec:mobility} confirm that the \textbf{Hybrid controller}---Analytical tracking at 100\,ms between VLA overrides at 822\,ms---outperforms both standalone methods: 145.5\,Mbps at 0\,km/h (vs.\ 128.7 Analytical, 117.1 VLA) and 93.0\,Mbps at 120\,km/h (within 3.3\% of the Optimized solver). The Hybrid inherits the VLA's superior static positioning quality while the Analytical component provides fast motion tracking. The standalone VLA retains the highest fairness ($>$0.94) at every speed; the Hybrid trades a small fairness reduction for a substantial throughput gain. TensorRT export, which could push VLA latency below 100\,ms, would narrow the decision-refresh gap and further strengthen the Hybrid's throughput advantage.
%
%A plausible operational flow: a UAV sits on a docking station at an RSU. When V2X signaling flags a coverage gap---a platoon approaching an underserved stretch---the RSU feeds user positions into the distilled model, gets a relay coordinate in ${\sim}$822\,ms, and dispatches the drone. The UAV holds that position until the platoon passes, then returns or receives an updated waypoint. At 822\,ms, the decision latency is short enough that the UAV's 4\,m/s transit speed is the binding constraint on repositioning, not the inference time.
%
%\subsection{Scalability to Multi-UAV and Multi-RSU Scenarios}
%
%The evaluation uses one UAV and one RSU; real deployments will have many of each. Scaling up brings two new issues: inter-UAV interference and coordinated spatial scheduling. On the distillation side the extension is conceptually direct: enlarge the prompt to list all RSU positions and existing UAV positions, generate optimizer labels for the joint problem, and retrain. Since the language model already handles variable-length token sequences (3-user and 7-user scenarios are just prompts of different length), no architectural change is needed. Whether 1.1B parameters suffice for, say, 10 UAVs serving 50 users is an open question not yet tested.
%
%\subsection{When to Use Distillation vs.\ Conventional ML}
%
%The results in Section~\ref{sec:mlp_drl} suggest a practical rule of thumb. When the input format is locked down and labeled data are plentiful, the MLP is hard to beat: 168.9\,Mbps at 1.1\,ms latency---and the MLP-2K experiment shows this holds even at the VLA's data budget. But if the scenario description is likely to change---new fields such as obstacle maps, weather, multi-RSU contexts---Section~\ref{sec:extensibility} shows the critical difference: under building obstructions, the blind MLP and blind VLA score similarly (137.8 vs.\ 135.1\,Mbps), but fine-tuning the VLA on just 500 obstacle-aware labels lifts it to 142.6\,Mbps---surpassing the blind MLP---while the MLP would require feature-vector redesign and full retraining. Zero-shot obstacle prompting (without fine-tuning) actually \emph{hurts} the VLA ($-$4.3\%), showing that the extensibility benefit requires targeted adaptation, not just prompt engineering. Among six alternative learning baselines, only SAC avoids collapse, reaching 114.5\,Mbps; PPO, TD3, and sum-pooling DeepSets fall to $\sim$68.5\,Mbps. Replacing sum-pooling with attention pooling lifts DeepSets to 93.2\,Mbps---still 30\% below the VLA. Even SAC trails the VLA by 17\%, confirming that neither model-free RL nor set-function architectures match the VLA's ability to preserve spatial semantics for continuous 3D positioning.
%
...
%    \item \textbf{Evolving scenario descriptions}: use the distilled VLA. When new context fields---obstacle maps, weather conditions, multi-RSU topologies---may enter the scenario description over the system's lifetime, the text interface absorbs them with minimal fine-tuning (500 labels, ${\sim}$16\,min). Section~\ref{sec:extensibility} shows that this yields a 5.6\% throughput gain over the blind VLA and surpasses the blind MLP.
%    \item \textbf{Mobile users, real-time tracking}: use the Hybrid controller (Analytical tracking at 100\,ms between VLA overrides at ${\sim}$822\,ms). Section~\ref{sec:mobility} shows this achieves 145.5\,Mbps at 0\,km/h and 93.0\,Mbps at 120\,km/h---outperforming both standalone methods and approaching the Optimized solver.
%    \item \textbf{Offline analysis, no latency constraint}: use the full evolutionary optimizer (189.3\,Mbps) as the quality ceiling.
%\end{itemize}
%This framing positions optimizer distillation not as a replacement for all methods but as the right tool when input extensibility and interpretability outweigh raw throughput or latency.
%
%\subsection{Limitations}
%
%Several limitations warrant discussion.
%
%\emph{Channel model.} The FSPL model with constant molecular absorption captures the dominant distance-dependent loss at 300\,GHz but omits Doppler spread, multipath fading, beam misalignment, and stochastic LoS/NLoS transitions (cf.\ 3GPP TR~36.777~\cite{3gpp36777}). Because the UAV operates above 10\,m, near-certain LoS is a reasonable assumption for the air-to-ground link, but the ground-to-RSU backhaul may experience intermittent blockage. Since all methods are evaluated under the same channel model, the \emph{relative} rankings should be preserved under a more realistic model; only the absolute throughput values would shift.
%
%\emph{Mobility.} Under motion the standalone VLA trails Analytical on throughput at every speed, though a Hybrid controller that combines both outperforms each standalone method (Section~\ref{sec:mobility}). The random-direction mobility model captures speed effects but not road-constrained motion; SUMO-based traces would add realism.
%
%\emph{Evaluation scale.} The evaluation comprises 100 test scenarios (25 per topology). While sufficient for the main claims (all paired $t$-tests reach $p < 0.01$), per-topology--per-user-count cells are thin (${\sim}$5 scenarios). A larger evaluation (e.g., 500 scenarios) would tighten confidence intervals for sub-group analyses.
%
%\emph{MLP throughput edge.} The MLP's throughput lead (170.2 vs.\ 134.0\,Mbps at matched 2K data) suggests the text interface costs something in raw positioning accuracy. Under building obstructions, however, this gap reverses: the obstacle-aware VLA (142.6\,Mbps) surpasses the blind MLP (137.8\,Mbps) once fine-tuned on 500 obstacle labels (Section~\ref{sec:extensibility}).
%
%\emph{Remaining.} The VLA's weakness in spread topologies (Section~\ref{sec:ablation}) points to a training-data diversity problem. The 2\% parse failure rate is avoidable with constrained decoding, as discussed in the overall-performance analysis of Section~\ref{sec:results}.
...
\bibitem{khabbaz2012}
M.~J. Khabbaz, C.~M. Assi, and W.~F. Fawaz, ``Disruption-tolerant networking: A comprehensive survey on recent developments and persisting challenges,'' \textit{IEEE Commun. Surveys Tuts.}, vol.~14, no.~2, pp.~607--640, 2012.
